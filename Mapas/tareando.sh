...
name="mapa.ps" # Nombre con el que se guardaran los mapas
...
do
	...
	awk -v num=$i 'NR==num {print $2 + 0.065, $3 - 0.055, $1}' "id_coords_stations.txt" | gmt pstext -R$r -J$j -P -F+f9p,Helvetica-Bold,black+jLM -Gwhite -O -K >> ${name}
done
...
#Nombre
echo -72.05 -29.9532 "Coquimbo" |  gmt pstext -R${r} -J${j}  -F+f8p,Times-Roman,black+jLM -Gwhite -O -K >> ${name}
echo -71.0683 -31.6327 "Illapel" |  gmt pstext -R${r} -J${j}  -F+f8p,Times-Roman,black+jLM -Gwhite -O -K >> ${name}

#Capital regional
echo "-71.252 -29.9027" | gmt psxy -R${r} -J${j} -Sc0.2C -W0.5p,0 -Ggreen  -O -K>> ${name}
echo -71.152 -29.9027 "La Serena" |  gmt pstext -R${r} -J${j}  -F+f10p,Times-Bold,black+jLM -Gwhite -O -K >> ${name}
...
echo -74.6 -30 "Capital Regional" |  gmt pstext -R${r} -J${j}  -F+f8p,Courier,black+jLM -Gwhite -O   >> ${name}
